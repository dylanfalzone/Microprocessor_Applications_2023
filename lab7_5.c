 ...
 *			when F is received, switch source to J3. When L is received, switch
 *			source back to cds. 
 *  Author: dylan
 */ 
...
ISR(USARTD0_RXC_vect){
	temp2 = usartd0_in_char();
	uint8_t received =temp2;
	
	if (received == 0x46){ //select j3
		ADCA.CH0.MUXCTRL = 0X00;
		ADCA.CH0.MUXCTRL = ADC_CH_MUXPOS_PIN5_gc | ADC_CH_MUXNEG_PIN4_gc;
	}
	if (received == 0x4c){ //select cds
		ADCA.CH0.MUXCTRL = 0X00;
		ADCA.CH0.MUXCTRL = ADC_CH_MUXPOS_PIN1_gc | ADC_CH_MUXNEG_PIN6_gc;
	}
